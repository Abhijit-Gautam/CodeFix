...
class KVCachedMultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.scale = self.head_dim ** 0.5  # Fixed: Use sqrt(head_dim)
    ...
    def forward(
        ...
        if cache is not None and cache.get('key') is not None:
            ...
            # Fixed: Cache concatenation on seq_len dimension (dim=1) before splitting heads
        ...
        # Fixed: Cache should store the full K, V (before splitting heads)
        # We need to merge heads back for storage, then split again on next forward
        new_cache = {
            'key': self._merge_heads(K),  # Store full cached K
            'value': self._merge_heads(V)  # Store full cached V
        }

        # Fixed: Cache size validation check - use >= for proper boundary
    ...
    def _split_heads(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Fixed: Correct reshape order - first to (batch, seq_len, num_heads, head_dim)
    ...
    def _compute_attention_scores(
        ...
        """
        # Fixed: Matrix multiplication - Q @ K^T with correct transpose
        scores = torch.matmul(Q, K.transpose(-2, -1))  # Fixed
    ...
    def _apply_causal_mask(
        ...
        # Fixed: Position offset - should be just cache_len (no +1)
        offset = cache_len  # Fixed

        # Create lower triangular mask: position i attends to j where j <= i
        # Fixed: Proper causal mask boundary condition
        mask = torch.ones(seq_len, batch_seq_len, device=scores.device)
        for i in range(seq_len):
            for j in range(batch_seq_len):
                if j <= i + offset:  # Fixed: Uses correct offset
                    mask[i, j] = 1
                else:
                    mask[i, j] = 0

        # Fixed: Mask dtype should be boolean for proper masked_fill operation
        mask = mask.bool()  # Fixed
