...
class BangaloreWumpusWorld:
    ...
    def get_cell_cost(self, x, y):
        """Get the cost of moving to a cell"""
        cell_type = self.grid[y][x]['type']
        
        if cell_type == 'pit':
            return float('inf')  # Avoid pits completely
        elif cell_type == 'traffic_light':
            return 20  # Traffic lights have high cost
        elif cell_type == 'cow':
            return float('inf')  # Avoid cows (they reset position)
        else:
            return self.grid[y][x]['weight']  # Use random weight for normal cells
    ...
    def find_path_astar(self):
        ...
        while open_set:
            ...
            if current == goal:
                ...
                for step in path:
                    g = g_score.get(step, 0)
                    h = self.heuristic(step, goal)
                    f = g + h
                    print(f"{step}: f(n) = g(n) + h(n) = {g} + {h} = {f}")
                
                self.message = "Path found! Navigating to goal..."
            ...
            for neighbor in neighbors:
                ...
                # Get cost to move to this neighbor
                move_cost = self.get_cell_cost(nx, ny)
                
                # Skip if impassable (pit or cow)
